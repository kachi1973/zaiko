...
use App\Models\KonyuItem;

class KonyuController extends Controller
{
    /**
    ...
    {
        return view('konyu.main', ['user' => Auth::user()]);
    }
    public function AjaxGetKonyus(Request $req){
        $perPage = 30;
        $pageNum = $req->pageNum;
        $q = Konyu::with(['xitems.xhinmoku', 'xstatus10user', 'xstatus20user', 'xstatus30user', 'xstatus40user', 'xstatus50user', 'xstatus60user', 'xstatus70user'])
            ->select("konyus.*")
            ->leftJoin('users', 'konyus.status10_user_id', '=', 'users.id')
        ...
        if($req->type == 'excel'){
            ...
            if(config('app.json_debug')){
                file_put_contents(base_path("bin\\konyus.json"), $json);
            }
            [$ret, $stdout, $stderr] = Helper::RunCmd(base_path("bin\\zaikocgi.exe 12"), $json);
            $headers = [
                'Content-Type' => 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                'Content-Disposition' => 'attachment; filename="konyus.xlsx"'
            ];
    ...
    public function AjaxGetKonyu(Request $req){
        return response()->json(
            [
                'item' => $this->get_konyu($req->id),
            ],
            200,
            [],
            JSON_UNESCAPED_UNICODE
    ...
    {
        $filename = "konyu.pdf";
        $proc = proc_open(base_path("bin\\zaikocgi.exe 4"), array(
            0 => array('pipe', 'r'),
            1 => array('pipe', 'w'),
            //2 => array('pipe', 'w')
        ), $pipes);
        ...
        $json = json_encode($this->get_konyu($req->id), 0);
        if(config('app.json_debug')){
            file_put_contents(base_path("bin\\konyu.json"), $json);
        }
        $len = strlen($json);
    ...
    public function AjaxPutKonyu(Request $req){
        $user = Auth::user();
        $id = 0;
        if(isset($req['item'])){
            $rs = $req['item'];
            ...
            if($rs['editable']){
                ...
                if (isset($rs['nouki_date'])) {
                    $s->nouki_date = Helper::ISO8601toDATE($rs['nouki_date']);
                }
                if (isset($rs['mitumori'])) {
                    $s->mitumori = $rs['mitumori'];
                }
                if (isset($rs['biko'])) {
                    $s->biko = $rs['biko'];
                }
                if (isset($rs['hachu_date'])) {
                    $s->hachu_date = Helper::ISO8601toDATE($rs['hachu_date']);
                }
                if (isset($rs['irai_no'])) {
                    $s->irai_no = $rs['irai_no'];
                }
                if (isset($rs['juchu_no'])) {
                    $s->juchu_no = $rs['juchu_no'];
                }
                if (isset($rs['kkamoku_id'])) {
                    $s->kkamoku_id = $rs['kkamoku_id'];
                }
            }
            $s->save();
            $id = $s['id'];
            if ($rs['editable']) {
                if (isset($rs['items'])) {
                    KonyuItem::where('konyu_id', $id)->delete();
